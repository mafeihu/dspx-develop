...
class Mall extends Common
{
    ...
    public function showMerchants(){
        if (Request::instance()->isPost()) {
            ...
            //获取商家
            $merchants_list = DB::name("merchants")->field('member_id,merchants_img,live_id,merchants_name')
    ...
    public function searchMerchants(){
        if (Request::instance()->isPost()) {
            ...
            $name && $where['merchants_name'] = ['like', '%' . $name . '%'];
            $where['is_delete'] = '0';
            $where['apply_state'] = '1';
            $where['pay_state'] = '1';
            ...
            switch ($type) {
                case 1:
                    $order = 'is_tuijian desc,create_time asc';
                    break;
                case 2:
                    $order = 'total_sales desc,create_time asc';
                    break;
                default :
                    $order = 'is_tuijian desc,create_time asc';
            }
            $count = Db::name('merchants')->where($where)->count();
            $merchants_list = DB::name("merchants")
                ->field('member_id,merchants_img,live_id,merchants_name,total_sales,merchants_content')
    ...
    public function showGoods(){
        if (Request::instance()->isPost()) {
            ...
            $list = Db::name('goods_class')
                ...
                ->where($where)->order("sort desc")->select();
            foreach ($list as &$v) {
                $v['show_goods'] = Db::name('goods_relation_class')->alias('a')
                    ...
                    ->where(['a.class_id' => $v['class_id'], 'b.is_delete' => '0', 'b.goods_state' => '1'])->order('b.sort desc,b.create_time asc')
                    ->select();
            }
...
//            }
    ...
    public function class_goods()
    {
        if (Request::instance()->isPost()) {
            ...
            $where = [
                'a.class_id' => $goodsClass['class_id'],
                'b.is_delete' => '0',
                'b.goods_state' => '1'
            ];
    ...
    public function merchants_info()
    {
        if (Request::instance()->isPost()) {
            ...
            $merchants = Db::name('merchants')->field('member_id,merchants_img,live_id,merchants_name,merchants_star1,merchants_star2,merchants_star3,merchants_content,total_sales')
                ->where($where)->find();
            if ($merchants) {
                if ($uid) {
                    $check = Db::name('follow')->where(['user_id' => $member['member_id'], 'user_id2' => $merchants_id,'is_delete'=>'1'])->find();
                    if ($check) {
    ...
    public function merchants_class_goods()
    {
        if (Request::instance()->isPost()) {
            ...
            $where = [
                'a.class_id' => $class['class_id'],
                'b.merchants_id' => $merchants_id,
                'b.is_delete' => '0',
                'b.goods_state' => '1'
            ];
            $list = Db::name('goods_relation_class')->alias('a')
    ...
    public function goods_info()
    {
        if (Request::instance()->isPost()) {
            ...
            /*商品基础信息*/
            $goods = Db::name('Goods')
                ->field('goods_id,goods_name,goods_img,imgs,code,goods_desc,goods_origin_price,goods_pc_price,goods_now_price,total_sales,month_sales,day_sales,goods_stock,goods_address,
            is_give_integral,give_integral_value,goods_star1,goods_star2,goods_star3,merchants_id,goods_detail')
    ...
    public function getShopCars(){
        if (Request::instance()->isPost()) {
            ...
            foreach ($list as $v) {
                ...
                if (!empty($goods)) {
                    if ($goods['goods_stock'] < $v['goods_num'] || $goods['is_delete'] == '1' || $goods['goods_state'] == '0') {
                        $no_valid[] = $v['car_id'];
                    } else {
                        $valid[] = $v['car_id'];
                    }
            ...
            Db::name('goods_shop_car')->where(['car_id' => ['in', $no_valid]])->update(['is_valid' => '1']);
            ...
            $no_valid_data = Db::name('goods_shop_car')->alias('a')
                ->field('a.car_id,a.goods_id,a.specification_id,a.goods_name,a.goods_num,a.goods_img,specification_names,b.goods_origin_price,b.goods_pc_price,b.goods_now_price')
                ->join('th_goods b', 'a.goods_id = b.goods_id')
                ->where(['a.member_id' => $member['member_id'], 'is_valid' => '1'])
                ->select();
