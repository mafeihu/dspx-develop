...
//            }
    ...
    public function give_gift(){
        ...
        $user_money = $user['b_diamond']-$count_price;       //用户扣除之后的金额
        $user_experience = $user['experience'] + $num*$gift['experience'];   //计算用户经验
        $give_gift_count = $user["give_gift_count"] + $count_price;
        $up_mebinfo = DB::name("member")->where(['member_id' =>$user['member_id']])->update(['give_gift_count'=>$give_gift_count,'b_diamond' => $user_money,'experience'=>$user_experience,'uptime' => time()]);
        if ($up_mebinfo) {
            $user_id2 = DB::name("live")->where(["live_id"=>$live_id])->value('user_id');    //主播id
            //进行主播判断(包括商家主播)
            $anchor = DB::name("anchor_info")->where(["anchor_id"=>$user_id2])->find();
            ...
            if($anchor || $merchants){
                if($anchor){
                    $dashang_scale = $anchor['dashang_scale']/100;//平台直播收益百分比
                    $tv_dashang_scale = $anchor['tv_dashang_scale']/100;//电视台直播收益百分比
                    $anchor_dashang_scale = 1-$dashang_scale-$tv_dashang_scale;//主播直播收益百分比
                    $tv_id = $anchor["tv_id"];
                    $member_type = 1;
                }else{
                    //电视台给商家的比例
                    $dashang_scale = $merchants['dashang_scale']/100;//平台直播收益百分比
                    $tv_dashang_scale = $merchants['tv_dashang_scale']/100;//电视台直播收益百分比
                    $anchor_dashang_scale = 1-$dashang_scale-$tv_dashang_scale;//主播直播收益百分比
                    $tv_id = $merchants["tv_id"];
                    $member_type = 2;
                }
                //电视台收益值
                $tv_money =$count_price*$change_scale*$tv_dashang_scale;
                //主播收益值
                $anchor_money = $count_price*$change_scale*$anchor_dashang_scale;
                //更新主播收益
                $anchor_get_gift_count = DB::name("member")->where(["member_id"=>$user_id2])->value('get_gift_count')+$anchor_money;
                $anchor_get_money = DB::name("member")->where(["member_id"=>$user_id2])->value('e_ticket') + $anchor_money;
                $amount =DB::name("member")->where(["member_id"=>$user_id2])->value("amount");
                $anchor_amount = sprintf("%.2f",($anchor_money*$cash_scale));
                //更新主播可兑换金额
                $anchor_get_amount = (float)$amount+(float)$anchor_amount;
                $up_anchor_money = DB::name("member")->where(['member_id' => $user_id2])->update(['get_gift_count'=>$anchor_get_gift_count,'e_ticket' => $anchor_get_money,'amount'=>$anchor_get_amount,'uptime' => time()]);
                ...
                $scale = $tv_dashang_scale.','.$anchor_dashang_scale;
                $mer_data = [
                    "user_id" => $user["member_id"],
                    "live_id"=>$live_id,
                    "user_id2"=>$user_id2,
                ...
                $tv_data = [
                    'anchor_id'=> $user_id2,
                    'tv_dashang_scale' =>$tv_dashang_scale,
                    "tv_id"=>$tv_id,
                    "member_type"=>$member_type,
                    "e_ticket" =>$tv_money,
                    "cash_money" =>(float)$cash_scale,
                ...
                $tv_res = DB::name("television_earnings")->insert($tv_data);
                $ticket_count = DB::name('Television')->where(['tv_id'=>$tv_id])->value('e_ticket_count')+$tv_money;
                DB::name('Television')->where(['tv_id'=>$tv_id])->update(['e_ticket_count'=>$ticket_count]);
                if ($tv_res){
                    DB::commit();
                    success('送礼成功!');
                }else{
                ...
                if($mers){
                    ...
                    //总后台设置的主播主播打赏打赏比
                    $anchor_scale = $system["dashang_scale"]/100;
                }
                //获取礼物转化为赏票数量
                $e_ticket = $count_price*$change_scale*$anchor_scale;
                //获取后台给主播分配的砖石数量
                $anchor_get_gift_count = DB::name('member')->where(['member_id'=>$user_id2])->value("get_gift_count")+$e_ticket;
                $get_money = DB::name("member")->where(["member_id"=>$user_id2])->value('e_ticket') + $e_ticket;
                $up_live_info = DB::name("member")->where(['member_id' => $user_id2])->update(['get_gift_count'=>$anchor_get_gift_count,'e_ticket' => intval($get_money),'uptime' => time()]);
                if (!$up_live_info){
                    Db::rollback();
                    error('点击太快!');
                }
...
//***************************************************************提现相关部分*********************************************************************************//
    ...
    public function tag_live_list(){
        ...
        if($count){
            $list = DB::name("live")->alias('a')
                ->field("a.title,a.live_id,a.play_img,a.watch_nums,a.room_id,a.play_address,a.play_address_m3u8,a.date,a.address,a.user_id,b.username,b.header_img")
                ->join('th_member b','a.user_id = b.member_id')
